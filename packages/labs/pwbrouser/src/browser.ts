  ...
  switch (action) {
    ...
    case 'dblclick':
      return {
        ...
        params: { target: { role: rest[0], name: rest[1] }, doubleClick: true },
      };
    ...
    case 'type':
      return {
        method: 'browser_type',
        params: { target: { role: rest[0], name: rest[1] }, text: rest[2] },
      };

    case 'fill':
      return {
        method: 'browser_fill',
        params: { target: { role: rest[0], name: rest[1] }, value: rest[2] },
      };
    ...
    case 'resize':
      return {
        ...
        params: { width: Number(rest[0]), height: Number(rest[1]) },
      };

    case 'wait_for':
      return {
        method: 'browser_wait_for',
        params: { [rest[0]!]: rest[0] === 'time' ? Number(rest[1]) : rest[1] },
      };
    ...
    case 'dialog': {
      const sub = rest[0];
      if (sub === 'accept')
        return { method: 'browser_handle_dialog', params: { accept: true, promptText: rest[1] } };
    ...
    case 'cookie': {
      ...
      if (sub === 'set')
        return { method: 'browser_cookie_set', params: { name: rest[1], value: rest[2] } };
    ...
    case 'localstorage': {
      ...
      if (sub === 'set')
        return { method: 'browser_localstorage_set', params: { key: rest[1], value: rest[2] } };
      if (sub === 'delete')
        return { method: 'browser_localstorage_delete', params: { key: rest[1] } };
      if (sub === 'clear') return { method: 'browser_localstorage_clear', params: {} };
      throw new Error(
        `Unknown localstorage action: ${sub}. Use get, set, delete, clear, or no subcommand for list.`,
      );
    }
    ...
    case 'sessionstorage': {
      ...
      if (sub === 'set')
        return { method: 'browser_sessionstorage_set', params: { key: rest[1], value: rest[2] } };
      if (sub === 'delete')
        return { method: 'browser_sessionstorage_delete', params: { key: rest[1] } };
      if (sub === 'clear') return { method: 'browser_sessionstorage_clear', params: {} };
      throw new Error(
        `Unknown sessionstorage action: ${sub}. Use get, set, delete, clear, or no subcommand for list.`,
      );
    }
    ...
    case 'tabs': {
      ...
      if (sub === 'select')
        return { method: 'browser_tabs', params: { action: 'select', index: Number(rest[1]) } };
...
export function sendRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', () => {
      reject(
        new Error(
          'Could not connect to browser daemon. Is the server running? Run: pwbrouser server start',
        ),
      );
    });
