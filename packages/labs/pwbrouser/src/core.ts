...
export async function browser_type(
  ...
  if (text === undefined || text === null)
    throw new Error('browser_type requires "text" parameter');
...
export async function browser_fill(
  ...
  if (value === undefined || value === null)
    throw new Error('browser_fill requires "value" parameter');
  const locator = resolveTarget(ctx.page, target);
  await locator.fill(value, { timeout: 5000 });
  return snap(ctx);
}

export async function browser_fill_form(
  ctx: MethodContext,
  params: Record<string, unknown>,
): Promise<MutatingResult> {
...
export async function browser_press_key(
  ctx: MethodContext,
  params: Record<string, unknown>,
): Promise<MutatingResult> {
...
export async function browser_resize(
  ...
  if (Number.isNaN(width) || width <= 0)
    throw new Error('browser_resize requires a valid "width" parameter');
  if (Number.isNaN(height) || height <= 0)
    throw new Error('browser_resize requires a valid "height" parameter');
  await ctx.page.setViewportSize({ width, height });
  return snap(ctx);
}

export async function browser_wait_for(
  ctx: MethodContext,
  params: Record<string, unknown>,
): Promise<MutatingResult> {
  ...
  const provided = [time !== undefined, text !== undefined, textGone !== undefined].filter(
    Boolean,
  ).length;
  if (provided !== 1)
    throw new Error('browser_wait_for requires exactly one of: time, text, textGone');
...
export async function browser_handle_dialog(
  ...
  const dialog = (
    ctx as unknown as {
      pendingDialog?: { accept: (text?: string) => Promise<void>; dismiss: () => Promise<void> };
    }
  ).pendingDialog;
  if (dialog) {
    if (accept) {
...
export async function browser_file_upload(
  ...
  const fileChooser = (
    ctx as unknown as {
      pendingFileChooser?: { setFiles: (paths: string[]) => Promise<void> } | null;
    }
  ).pendingFileChooser;
