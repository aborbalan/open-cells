...
async function initBrowser(): Promise<void> {
  try {
    ...
    console.error(
      `[Daemon Error] Failed to launch browser: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  }
...
const server = http.createServer(async (req, res) => {
  ...
  req.on('end', async () => {
    try {
      ...
      try {
        snapshot =
          (await (page.ariaSnapshot as (opts?: { ref?: boolean }) => Promise<string | null>)({
            ref: true,
          })) ?? undefined;
      } catch {
        /* ignore */
