...
describe("browser_navigate", () => {
  it("navigates to URL and returns snapshot", async () => {
    ...
    expect(c.page.goto).toHaveBeenCalledWith("https://example.com", { waitUntil: "networkidle", timeout: 60000 });
...
describe("browser_click", () => {
  ...
  it("passes button and modifiers to click", async () => {
    ...
    expect(clickFn).toHaveBeenCalledWith({ button: "right", modifiers: ["Control"], timeout: 5000 });
  });
...
describe("browser_drag", () => {
  ...
  it("throws if startTarget is missing", async () => {
    await expect(browser_drag(ctx(), { endTarget: { ref: "e2" } } as never)).rejects.toThrow('"startTarget"');
  });

  it("throws if endTarget is missing", async () => {
    await expect(browser_drag(ctx(), { startTarget: { ref: "e1" } } as never)).rejects.toThrow('"endTarget"');
  });
});
...
describe("browser_fill", () => {
  ...
  it("throws if value is missing", async () => {
    await expect(browser_fill(ctx(), { target: { ref: "e1" } } as never)).rejects.toThrow('"value"');
  });
});
...
describe("browser_select_option", () => {
  ...
  it("throws if target is missing", async () => {
    await expect(browser_select_option(ctx(), { values: ["x"] } as never)).rejects.toThrow('"target"');
  });

  it("throws if values is empty", async () => {
    await expect(browser_select_option(ctx(), { target: { ref: "e1" }, values: [] })).rejects.toThrow('"values"');
  });
});
...
describe("browser_resize", () => {
  ...
  it("throws if width is not a number", async () => {
    await expect(browser_resize(ctx(), { width: "abc", height: 600 } as never)).rejects.toThrow('"width"');
  });
});
...
describe("browser_wait_for", () => {
  ...
  it("waits for text to disappear", async () => {
    ...
    expect(c.page.waitForSelector).toHaveBeenCalledWith("text=Loading...", { state: "detached", timeout: 5000 });
  });
