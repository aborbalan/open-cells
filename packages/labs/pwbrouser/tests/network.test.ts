...
function makeMockRoute() {
  return {
    ...
    request: () => ({
      headers: () => ({ cookie: "session=abc", "user-agent": "test", "content-type": "application/json" }),
    }),
  };
}
...
describe("browser_route", () => {
  ...
  it("handler fulfills with provided status and body", async () => {
    ...
    c.page.route = vi.fn().mockImplementation((_pattern: string, handler: (route: unknown) => Promise<void>) => {
      capturedHandler = handler;
    });

    await browser_route(c, { pattern: "**/api/*", status: 201, body: '{"ok":true}', contentType: "application/json" });
...
describe("browser_network_state_set", () => {
  ...
  it("throws if state is invalid", async () => {
    await expect(browser_network_state_set(ctx(), { state: "airplane" })).rejects.toThrow("Invalid network state");
  });
});
